...
public class SineWaveDevice extends SimulatedAppliance {

	private double maxAmplitude;
	private double wavelength;
	
	protected void applianceSpecificInitialization() {
		maxAmplitude = Math.random() * (float)200;
		wavelength = Math.random() * 60 * 60;
	}

	public SimulatedAppliance copy() {
		return new SineWaveDevice();
	}
	...
	protected int powerDrawAtOnCycleTimestep(int totalSeconds, int secondIndex, int globalTimestep) {
		return (int)(Math.sin((2 * Math.PI) * (float)secondIndex / wavelength) * maxAmplitude) + (int)Math.ceil(maxAmplitude);
	}
