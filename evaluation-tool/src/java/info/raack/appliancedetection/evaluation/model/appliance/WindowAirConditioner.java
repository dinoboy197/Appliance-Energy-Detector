...
@Component
public class WindowAirConditioner extends TwoStatePowerDrawAppliance {

	protected double getHighFraction(int totalSeconds) {
		// compressor is on (high) 50% of the time
		return 0.5d;
	}
	...
	protected void applianceSpecificInitialization() {
		...
		// min power = 1000 - 3000
		// cycle time = 10 - 20
		minPowerDraw = (int)(80 + Math.random() * (float)50);
		maxPowerDraw = (int)(1000 + Math.random() * (float)2000);
		cycleTime = (int)(10 + Math.random() * 10.0f);
	}

	public SimulatedAppliance copy() {
		return new WindowAirConditioner();
	}
