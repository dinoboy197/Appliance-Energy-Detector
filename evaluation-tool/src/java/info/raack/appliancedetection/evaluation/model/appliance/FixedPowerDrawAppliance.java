...
public abstract class FixedPowerDrawAppliance extends SimulatedAppliance {

	protected int powerDraw = 0;

	protected int powerDrawAtOnCycleTimestep(int totalSeconds, int secondIndex, int globalTimestep) {
		// flat power draw
		
		if(secondIndex < cycleTransitionRampDuration) {
			// check for ramp up
			return (int)((float)powerDraw * ((float)secondIndex / (float)cycleTransitionRampDuration));
		} else if(totalSeconds - secondIndex <= cycleTransitionRampDuration) {
			// ramp down
			return (int)((float)powerDraw * ((float)(totalSeconds - secondIndex) / (float)cycleTransitionRampDuration));
		} else {
			return powerDraw;
		}
	}
