...
public class AlgorithmResultTest {
	...
	@Test public void contructorAndGettersWorkCorrectly() {
		Serializable a = mock(Serializable.class);
		ApplianceEnergyConsumptionDetectionAlgorithm algorithm = mock(ApplianceEnergyConsumptionDetectionAlgorithm.class);
		EnergyMonitor monitor = mock(EnergyMonitor.class);
		
		AlgorithmResult result = new AlgorithmResult(monitor, algorithm, a);
		
		assertEquals(a, result.getResult());
		assertEquals(algorithm, result.getAlgorithm());
		assertEquals(monitor, result.getMonitor());
	}
}
