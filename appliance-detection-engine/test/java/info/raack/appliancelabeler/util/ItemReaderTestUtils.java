 ...
 ******************************************************************************/
package info.raack.appliancelabeler.util;
...
public class ItemReaderTestUtils {

	public static ItemReader<SecondData> createSecondDataReader(Calendar startTime, int[] powerMeasurements) {
		List<SecondData> data = new ArrayList<SecondData>();
		Calendar cal = (Calendar)startTime.clone();
		for(int powerMeasurement : powerMeasurements) {
			SecondData measurement = new SecondData();
			measurement.setCal(cal);
			measurement.setPower(powerMeasurement);
			data.add(measurement);
			cal.add(Calendar.SECOND,1);
		}
		
		return new ListItemReader<SecondData>(data);
	}
}
