...
public class FrequencyResponseApplianceState implements ApplianceState {

	private FrequencyResponseStateTransition transition;
	
	public FrequencyResponseApplianceState(FrequencyResponseStateTransition transition) {
	...
	public int getCurrentPower() {
		...
		// the the largest wavelength amplitude, and return it's square root
		double current = 0;
		for(double amp : transition.getVariableValues()) {
			current = Math.max(amp, current);
		}
	...
	@Override
	public boolean isOn() {
		return transition.isOn();
	}

	@Override
	public void createPowerShutoff() {
		transition = new FrequencyResponseStateTransition(-1, transition.getUserAppliance(), -2, false, transition.getTime(), new double[]{});
	}
