 ...
 ******************************************************************************/
package info.raack.appliancelabeler.machinelearning.appliancedetection.algorithmcomponents;
...
public abstract class IterativeSecondDataProcessor<T> {

	private LinkedList<SecondData> lastMeasurements = new LinkedList<SecondData>();
	private int measurementThreshold;
	
	public IterativeSecondDataProcessor(int measurementThreshold) {
		this.measurementThreshold = measurementThreshold;
		lastMeasurements.add(null);
	}
	
	public T detectCurrentTransition(SecondData currentPoint) {
		// add current point onto the back of the list
		lastMeasurements.add(currentPoint);
		
		if(lastMeasurements.size() > measurementThreshold) {
			// remove the last point from the list
			lastMeasurements.pop();
			
			return detectCurrentTransitionInternal(lastMeasurements);
		} else {
			return null;
		}
	}
		
	protected abstract T detectCurrentTransitionInternal(List<SecondData> lastMeasurements);
}
